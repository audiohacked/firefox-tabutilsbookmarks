...
let TabmixSvc = {
  ...
  getString: function(aStringKey) {
    try {
      ...
      dump("*** Failed to get string " + aStringKey + " in bundle: tabutilsbookmarks.properties\n");
      throw e;
    }
  },
  ...
  getFormattedString: function(aStringKey, aStringsArray) {
    try {
      ...
      dump("*** Failed to format string " + aStringKey + " in bundle: tabutilsbookmarks.properties\n");
      throw e;
    }
  },
  ...
  windowStartup: {
    ...
    init: function(aWindow) {
      ...
      try {
        ...
        // we must call this before any other tabutilsbookmarks function
      ...
      if (isVersion(190))
        Cu.import("resource://tabutilsbookmarks/DownloadLastDir.jsm");

      Cu.import("resource://tabutilsbookmarks/Places.jsm");
      TabmixPlacesUtils.init(aWindow);

      TabmixSvc.tabStylePrefs = {};
      let tmp = {};
      Cu.import("resource://tabutilsbookmarks/DynamicRules.jsm", tmp);
...
XPCOMUtils.defineLazyGetter(TabmixSvc, "prefBranch", function () {return Services.prefs.getBranch("extensions.tabutilsbookmarks.")});
// string bundle
XPCOMUtils.defineLazyGetter(TabmixSvc, "_strings", function () {
  let properties = "chrome://tabutilsbookmarks/locale/tabutilsbookmarks.properties";
  return Services.strings.createBundle(properties);
});
XPCOMUtils.defineLazyGetter(TabmixSvc, "SMstrings", function () {
  let properties = "chrome://tabutilsbookmarks/locale/session-manager.properties";
  return Services.strings.createBundle(properties);
});
...
XPCOMUtils.defineLazyModuleGetter(TabmixSvc, "console",
  "resource://tabutilsbookmarks/log.jsm");
